...
from bot import GDRIVE_ID, IS_TEAM_DRIVE, INDEX_URL, USE_SERVICE_ACCOUNTS, VIEW_LINK, \
...
class GoogleDriveHelper:
            ...
            try:
                ...
                if err.resp.get('content-type', '').startswith('application/json'):
                    ...
                    if USE_SERVICE_ACCOUNTS:
                        ...
                        return self.__upload_file(file_path, file_name, mime_type, GDRIVE_ID)
                    else:
    ...
    def __cloneFolder(self, name, local_path, folder_id, GDRIVE_ID):
        ...
        for file in files:
            if file.get('mimeType') == self.__G_DRIVE_DIR_MIME_TYPE:
                ...
                self.__copyFile(file.get('id'), GDRIVE_ID)
            if self.__is_cancelled:
                break
    ...
    def drive_list(self, fileName, stopDup=False, noMulti=False, isRecursive=True, itemType=""):
        ...
        for index, GDRIVE_ID in enumerate(DRIVES_IDS):
            isRecur = False if isRecursive and len(GDRIVE_ID) > 23 else isRecursive
            response = self.__drive_query(GDRIVE_ID, fileName, stopDup, isRecur, itemType)
            ...
            for file in response.get('files', []):
                ...
                if mime_type == "application/vnd.google-apps.folder":
                    ...
                    if INDEX_URLS[index] is not None:
                        if isRecur:
                            url_path = "/".join([rquote(n, safe='') for n in self.__get_recursive_list(file, GDRIVE_ID)])
                        else:
                            url_path = rquote(f'{file.get("name")}', safe='')
